...
"""
========================================
v8.1 混合信号分级调仓版红利策略监测脚本
========================================
功能流程：数据获取 → 因子计算 → 评分 → 风控 → 权重重分配 → ERP现金管理
         → 混合信号调仓判断(L1/L2/L3/时间保底) → DDCA检查 → 报告推送

调仓方案（v8.1核心改进）：
  - 建仓期：每2周检查一次（不变）
  - 持有期：每10交易日综合检查
    · L1: 权重偏离5/25规则连续3天 → 单标的再平衡 + 7日冷却
    · L2: 组合估值分位极端(>85%或<15%)连续5天 → 全组合再平衡
    · L3: 基本面红线 → 立即清仓（由风控模块处理）
    · 时间保底: 每年1月/7月强制全面检查
"""
...
STATIC_FALLBACK_DATA = {
    ...
    "600011": {
        "name": "华能国际", "price": 7.2, "dividend_yield": 0.032,
        "roe": 0.05, "roe_std_3y": 0.025, "cashflow_to_debt": 0.12,
        "pe_ttm": 15.0, "volatility_250d": 0.22, "earnings_growth": -0.02,
        "net_profit_history": [80e8, 65e8, 70e8],
        "operating_cashflow_history": [50e8, -10e8, 20e8],
...
class DividendMonitor:
    ...
    def generate_report(self) -> str:
        ...
        lines = []
        lines.append("=" * 40)
        lines.append(f"v8红利策略监测日报 | {today_str}")
        lines.append("=" * 40)
        lines.append("")
        ...
        for code in sorted(self.score_results.keys()):
            ...
            lines.append(f"  {code} {sr.name}: 评分={sr.total_score:.1f} 目标仓位={target_w:.0%} 当前权重={current_w:.1%} DY={fd.dividend_yield:.2%} {tax_hint}")
        lines.append("")

        lines.append("【风控信号】")
        if self.risk_signals:
            for s in self.risk_signals:
                lines.append(f"  [{'清仓' if s.signal_type=='clear' else '减仓' if s.signal_type=='reduce' else s.signal_type}] {s.code}: {s.reason}")
        else:
            lines.append("  无风控信号")
        lines.append("")
        ...
        if all_signals:
            for ts in all_signals:
                lines.append(f"  [{ts.action.upper()}] {ts.code} {ts.name}: {ts.reason}, 建议金额={ts.suggested_amount:,.0f}元")
        else:
            lines.append("  无交易信号")
        lines.append("")
